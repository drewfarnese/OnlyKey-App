 ...
 * the user to download instead of being streamed to a temp dir. The browser
 * download means there is no in-app binary to hash-verify; the manifest and
 * package URLs are still required to be HTTPS.
 */
...
export async function checkForAppUpdate(io: AppUpdateIo = {}): Promise<void> {
  ...
  try {
    ...
    const manifestUrl = pkg.manifestUrl ?? DEFAULT_MANIFEST_URL;
